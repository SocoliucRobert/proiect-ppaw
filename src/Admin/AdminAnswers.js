...
const AdminAnswers = () => {
  ...
  const handleAddAnswer = async () => {
    ...
    try {
      const { data, error } = await supabase
        .from('answers')
        .insert([{ question_id: parseInt(questionId, 10), answer_text: answerText,is_correct:isCorrect }])
        .select();
  ...
  {questions.map((q) => (
    <option key={q.id} value={q.id}>
      {q.question_text}
            ...
            {editingAnswerId ? (
              <button onClick={handleUpdateAnswer}>Actualizează Răspunsul</button>
            ) : (
  ...
  {answers.map((answer) => (
    <div key={answer.id} className={styles.answerItem}>
      ...
      <div className={styles.actions}>
        <button onClick={() => handleEditToggle(answer.id)}>Editează</button>
        <button onClick={() => handleDeleteAnswer(answer.id)}>Șterge</button>
      </div>
    </div>
  ))}
</div>
